 ...
 *
 */
public class SciMemoryNetwork implements GraphicalComponent {
	 ...
	 */
	public SciMemoryNetwork(boolean draw){
		drawGraph = draw;
		init();		
	}
	 ...
	 * @param draw set true if graphics are necessary (slow)
	 */
	public SciMemoryNetwork(String name, boolean draw){
		this.name = name;
		drawGraph = draw;
		init();
	}
	 ...
	 * @param draw set true if graphics are necessary (slow)
	 */
	public SciMemoryNetwork(int nn, int ni, double wr, double maxw, double nr, String name, boolean draw){
		numberOfNeurons = nn;
	...
	private void init(){
		...
		neurons = new IAFNeuron[numberOfNeurons];
...
//	   
		...
		for(int i=0; i<numberOfNeurons; i++){
			if(Constants.uniformDouble()<=0.7){
				neurons[i].addToI(noise+noiseRand.nextGaussian());//noise*noiseRand.nextGaussian());
			}
	        for(int j=0; j<numberOfNeurons; j++){
